...
if selected == 'Diabetes Prediction':
    ...
    st.markdown("""
    <div class="info-box">
        <p style= "margin-top: 0.2rem; padding-top: 0.1rem">Enter your health parameters below to assess your diabetes risk. Our AI model will analyze these metrics to determine your risk level.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Number of pregnancies</p>", unsafe_allow_html=True)
        Pregnancies = st.text_input('', '0', key='preg')
    
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Glucose level</p>", unsafe_allow_html=True)
        Glucose = st.text_input('', '0', key='glu')
    
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Blood pressure value</p>", unsafe_allow_html=True)
        BloodPressure = st.text_input('', '0', key='bp')
    
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Skin Thickness value</p>", unsafe_allow_html=True)
        SkinThickness = st.text_input('', '0', key='skin')
    
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Insulin level</p>", unsafe_allow_html=True)
        Insulin = st.text_input('', '0', key='insulin')
    
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>BMI value</p>", unsafe_allow_html=True)
        BMI = st.text_input('', '0', key='bmi')
    
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Diabetes Pedigree Function value</p>", unsafe_allow_html=True)
        DiabetesPedigreeFunction = st.text_input('', '0', key='dpf')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Age of the Person</p>", unsafe_allow_html=True)
    ...
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Age</p>", unsafe_allow_html=True)
        age = st.number_input('', min_value=0, max_value=120, key='h_age')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>(1=Male, 0=Female)</p>", unsafe_allow_html=True)
        sex = st.number_input('', min_value=0, max_value=1, key='h_sex')
        
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Chest Pain types</p>", unsafe_allow_html=True)
        cp = st.number_input('', min_value=0, max_value=3, key='h_cp')
        
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Resting Blood Pressure</p>", unsafe_allow_html=True)
        trestbps = st.number_input('', min_value=0, key='h_bp')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Serum Cholestoral in mg/dl</p>", unsafe_allow_html=True)
        chol = st.number_input('', min_value=0, key='h_chol')
        
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Fasting Blood Sugar > 120 mg/dl</p>", unsafe_allow_html=True)
        fbs = st.number_input('', min_value=0, max_value=1, key='h_fbs')
        
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Resting Electrocardiographic results</p>", unsafe_allow_html=True)
        restecg = st.number_input('', min_value=0, max_value=2, key='h_restecg')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Maximum Heart Rate achieved</p>", unsafe_allow_html=True)
        thalach = st.number_input('', min_value=0, key='h_thalach')
        
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Exercise Induced Angina</p>", unsafe_allow_html=True)
        exang = st.number_input('', min_value=0, max_value=1, key='h_exang')
        
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>ST depression induced by exercise</p>", unsafe_allow_html=True)
        oldpeak = st.number_input('', key='h_oldpeak')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Slope of the peak exercise ST segment</p>", unsafe_allow_html=True)
        slope = st.number_input('', min_value=0, max_value=2, key='h_slope')
        
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Major vessels colored by flourosopy</p>", unsafe_allow_html=True)
        ca = st.number_input('', min_value=0, max_value=4, key='h_ca')
        
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>thal: 0 = normal; 1 = fixed defect; 2 = reversable defect</p>", unsafe_allow_html=True)
...
# Parkinson's Prediction Page
    ...
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:Fo(Hz)</p>", unsafe_allow_html=True)
        fo = st.text_input('', '0', key='p_fo')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:Fhi(Hz)</p>", unsafe_allow_html=True)
        fhi = st.text_input('', '0', key='p_fhi')
        
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:Flo(Hz)</p>", unsafe_allow_html=True)
        flo = st.text_input('', '0', key='p_flo')
        
    with col4:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:Jitter(%)</p>", unsafe_allow_html=True)
        Jitter_percent = st.text_input('', '0', key='p_jit_pct')
        
    with col5:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:Jitter(Abs)</p>", unsafe_allow_html=True)
        Jitter_Abs = st.text_input('', '0', key='p_jit_abs')
        
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:RAP</p>", unsafe_allow_html=True)
        RAP = st.text_input('', '0', key='p_rap')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:PPQ</p>", unsafe_allow_html=True)
        PPQ = st.text_input('', '0', key='p_ppq')
        
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Jitter:DDP</p>", unsafe_allow_html=True)
        DDP = st.text_input('', '0', key='p_ddp')
        
    with col4:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:Shimmer</p>", unsafe_allow_html=True)
        Shimmer = st.text_input('', '0', key='p_shimmer')
        
    with col5:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:Shimmer(dB)</p>", unsafe_allow_html=True)
        Shimmer_dB = st.text_input('', '0', key='p_shimmer_db')
        
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Shimmer:APQ3</p>", unsafe_allow_html=True)
        APQ3 = st.text_input('', '0', key='p_apq3')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Shimmer:APQ5</p>", unsafe_allow_html=True)
        APQ5 = st.text_input('', '0', key='p_apq5')
        
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>MDVP:APQ</p>", unsafe_allow_html=True)
        APQ = st.text_input('', '0', key='p_apq')
        
    with col4:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>Shimmer:DDA</p>", unsafe_allow_html=True)
        DDA = st.text_input('', '0', key='p_dda')
        
    with col5:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>NHR</p>", unsafe_allow_html=True)
        NHR = st.text_input('', '0', key='p_nhr')
        
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>HNR</p>", unsafe_allow_html=True)
        HNR = st.text_input('', '0', key='p_hnr')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>RPDE</p>", unsafe_allow_html=True)
        RPDE = st.text_input('', '0', key='p_rpde')
        
    with col3:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>DFA</p>", unsafe_allow_html=True)
        DFA = st.text_input('', '0', key='p_dfa')
        
    with col4:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>spread1</p>", unsafe_allow_html=True)
        spread1 = st.text_input('', '0', key='p_spread1')
        
    with col5:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>spread2</p>", unsafe_allow_html=True)
        spread2 = st.text_input('', '0', key='p_spread2')
        
    with col1:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>D2</p>", unsafe_allow_html=True)
        D2 = st.text_input('', '0', key='p_d2')
        
    with col2:
        st.markdown("<p class='input-label' style='margin-bottom: -1rem'>PPE</p>", unsafe_allow_html=True)
